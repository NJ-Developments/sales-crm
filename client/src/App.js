...
export default function App() {
  ...
  function search() {
    ...
    service.textSearch({ query: query.trim() }, (results, status) => {
      setSearching(false);
      if (status !== 'OK' || !results) return alert('No results found');
      ...
      if (results[0]) { mapInstance.current.setCenter(results[0].geometry.location); mapInstance.current.setZoom(12); }
      setTab('leads');
    });
  }
